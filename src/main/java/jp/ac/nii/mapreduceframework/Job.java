...
public class Job<InKey, InValue, InterKey extends Comparable<InterKey>, InterValue, OutKey, OutValue> {
	...
	private final List<URI> cacheFiles = Lists.newArrayList();

	private Job(Configuration conf) {
		this.conf = conf;
	}
	...
	public static <InKey, InValue, InterKey extends Comparable<InterKey>, InterValue, OutKey, OutValue> Job<InKey, InValue, InterKey, InterValue, OutKey, OutValue> getInstance() {
		return new Job<InKey, InValue, InterKey, InterValue, OutKey, OutValue>(new Configuration());
	}

	public static <InKey, InValue, InterKey extends Comparable<InterKey>, InterValue, OutKey, OutValue> Job<InKey, InValue, InterKey, InterValue, OutKey, OutValue> getInstance(
			Configuration conf) {
		return new Job<InKey, InValue, InterKey, InterValue, OutKey, OutValue>(conf);
	}
	...
	private void mapParallel(final Shuffler<InterKey, InterValue> shuffler,
		...
		for (int i = 0; i < mappers.size(); i++) {
			...
			for (final Pair<InKey, InValue> pair : inputData.getData()) {
				Context context = createContextForMapper(shuffler, inputData);
	...
	private Context createContextForMapper(final Shuffler<InterKey, InterValue> shuffler,
			final InputData<InKey, InValue> inputData) {
		return new Context() {
	...
	private void reduceParallel(Shuffler<InterKey, InterValue> shuffler,
		...
		for (int i = 0; i < reducers.size(); i++) {
			...
			for (Entry<InterKey, List<InterValue>> keyValue : keyValueMap.entrySet()) {
				Context context = createContextForReducer(outputFormat);
	...
	private Context createContextForReducer(final OutputFormat outputFormat) {
		return new Context() {
			@SuppressWarnings("unchecked")
			@Override
			public void write(Object key, Object value) {
